...
function Todos($element) {
  ...
  $el.on('click', 'input[type=checkbox]', function() {
    hoodie.store.remove($(this).parent().data('id'));
    return false;
  });
  ...
  $el.on('keypress', 'input[type=text]', function(event) {
    if (event.keyCode === 13) {
      hoodie.store.update($(this).parent().data('id'), {title: event.target.value});
    }
  });
...
hoodie.store.findAll().then(function(allTodos) {
  allTodos.forEach(todos.add);
});

// when a todo changes, update the UI.
hoodie.store.on('add', todos.add);
hoodie.store.on('update', todos.update);
hoodie.store.on('remove', todos.remove);
...
$('#todoinput').on('keypress', function(event) {
  ...
  if (event.keyCode === 13 && event.target.value.length) {
    hoodie.store.add({title: event.target.value});
    event.target.value = '';
  }
});
